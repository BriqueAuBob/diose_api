...
export default class SuggestionsController {
  public async store({ auth, request }) {
    ...
    await request.validate({
      ...
      messages: {
        ...
        minLength:
          "Le contenu de la suggestion dois faire au moins {{ options.minLength }} caractères.",
        maxLength:
          "Le contenu de la suggestion dois faire  {{ options.maxLength }} caractères au maximum.",
      },
    });
    ...
    await axios.put(
      `https://discordapp.com/api/v8/channels/${data.channel_id}/messages/${
        data.id
      }/reactions/${encodeURI("✅")}/@me`,
      {},
      {
        headers: {
          Authorization: "Bot " + Config.get("discord.BOT_TOKEN"),
  ...
  async index() {
    ...
    if (suggestions.length <= 0) {
      ...
      await Redis.set(
        "suggestions",
        JSON.stringify(messagesSuggestions),
        "EX",
        60
      );
